// Capture the Chrome oracle for Worklet entry points.
//
...
const server = createServer((request, response) => {
  ...
  // Every other path 404s on purpose, including /definitely-missing-worklet.js:
  // the probe uses it to observe addModule failure.
...
const profileDir = await mkdtemp(join(tmpdir(), 'obscura-chrome-worklet-'));
...
try {
  ...
  const evaluated = await cdp.send('Runtime.evaluate', {
    expression: 'workletFixturePromise',
    awaitPromise: true,
    returnByValue: true,
  }, sessionId);
