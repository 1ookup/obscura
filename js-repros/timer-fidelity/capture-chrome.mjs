...
const fixtureUrl = 'data:text/html;charset=utf-8,' + encodeURIComponent(
  `<!doctype html><meta charset="utf-8"><body><script>${probe}</script>`,
);
const profileDir = await mkdtemp(join(tmpdir(), 'obscura-chrome-timers-'));
...
try {
  ...
  const evaluated = await cdp.send('Runtime.evaluate', {
    expression: 'timerFixturePromise',
    awaitPromise: true,
    returnByValue: true,
  }, sessionId);
