// Capture the Chrome oracle for SharedWorker semantics.
//
// A SharedWorker needs a same-origin script URL, so the probe and the worker
// script are both served over loopback HTTP rather than from a data: URL.
...
const workerSource = await readFile(join(fixtureDir, 'shared-worker.js'), 'utf8');
...
const server = createServer((request, response) => {
  ...
  if (request.url.startsWith('/shared-worker.js')) {
...
const profileDir = await mkdtemp(join(tmpdir(), 'obscura-chrome-shared-'));
...
try {
  ...
  const evaluated = await cdp.send('Runtime.evaluate', {
    expression: 'sharedWorkerFixturePromise',
    awaitPromise: true,
    returnByValue: true,
  }, sessionId);
