...
mod tests {
    ...
    #[test]
    fn iframe_content_closed_shadow_tree_composites_into_parent() {
        let tree = parse_html(
            "<!DOCTYPE html><html><body style=\"margin:0\"><iframe style=\"display:block;border:0;width:100px;height:50px\"></iframe></body></html>",
        );
        let frame_host = tree.query_selector("iframe").unwrap().unwrap();
        let (content_root, _) = tree.create_iframe_content_document(frame_host).unwrap();
        obscura_dom::parse_into_subtree(
            &tree,
            content_root,
            "<!DOCTYPE html><html><body id=frame-body style=\"margin:0\"></body></html>",
        );
        let frame_body = tree
            ...
            .unwrap();
        let shadow_root = tree
            .attach_shadow_root(frame_body, ShadowRootMode::Closed)
            .unwrap();
        obscura_dom::parse_into_subtree(
            &tree,
            shadow_root,
