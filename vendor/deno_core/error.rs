...
impl JsStackFrame {
  ...
  fn from_callsite_object<'s>(
    ...
    // Read all CallSite values before touching the source-map RefCell. V8 may
    // re-enter stack formatting from one of those methods; holding the mapper
    // during the calls would panic on a nested borrow.
    let (file_name, line_number, column_number) = match (
      ...
      (Some(f), Some(l), Some(c)) => {
        // CallSite formatting runs from V8's prepare-stack callback. Mapping
        // there can re-enter the loader while V8 owns the stack handles, so
        // retain the raw location for the caller.
        (Some(f), Some(l), Some(c))
      }
      (f, l, c) => (f, l, c),
    };

    // As with the primary location, leave eval origins untouched while V8 is
    // formatting the stack. Source-map presentation is optional metadata.
    let eval_origin = call!(GET_EVAL_ORIGIN: Option<String>);
  ...
  pub fn from_v8_message<'a>(
    ...
    // Source-map lookup is optional during nested error formatting. Raw V8
    // locations remain valid when another formatter owns the mapper.
    let (file_name, line_num, col_num) = (f, l, c);
...
impl JsError {
  ...
  pub fn from_v8_message<'a>(
    ...
    }
    {
      ...
      if let Ok(mut source_mapper) = state.source_mapper.try_borrow_mut() {
  ...
  fn inner_from_v8_exception<'a>(
    ...
    if is_instance_of_error(scope, exception) {
      ...
      let source_line_frame_index = None;
      ...
      // Source-line lookup is intentionally omitted here. Error.stack is
      // materialized through the embedder callback, and asking the loader for
      // presentation metadata from this conversion can re-enter the isolate.
...
pub(crate) fn exception_to_err_result<T>(
  ...
  // A terminated isolate may carry an incomplete V8 exception/stack object.
  // Walking its CallSite frames while termination is being resumed can enter
  // the source mapper re-entrantly and, on some V8 revisions, dereference a
  // frame that no longer exists. Termination is a control-flow outcome, not a
  // page exception, so return a stable synthetic error without touching the
  // stack formatter.
  let mut js_error = if was_terminating_execution {
    JsError {
      name: Some("Error".to_string()),
      message: Some("execution terminated".to_string()),
      stack: Some("Error: execution terminated".to_string()),
      cause: None,
      exception_message: "Uncaught Error: execution terminated".to_string(),
      frames: vec![],
      source_line: None,
      source_line_frame_index: None,
      aggregated: None,
...
pub mod callsite_fns {
  ...
  fn maybe_apply_source_map<'a>(
    ...
    let state = JsRuntime::state_from(scope);
    let Ok(mut source_mapper) = state.source_mapper.try_borrow_mut() else {
      return Some((file_name, line_number, column_number));
    };
